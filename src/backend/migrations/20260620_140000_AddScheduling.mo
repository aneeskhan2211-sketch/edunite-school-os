// Migration: Add scheduling storage (enrollments + classMeetings + counter)
// OldActor = NewActor of 20260618_060000_AddGradebookStorage.mo
...
module {
  ...
  // ── Shared types (inlined) — must exactly match previous NewActor ─
  type Role = {
  ...
  type CommitmentStatus = { #open; #inProgress; #completed; #cancelled; #overdue };
  ...
  type Commitment = {
    ...
    status : CommitmentStatus; notes : Text;
  ...
  type AssignmentV2 = { id : Nat; classId : CourseId; categoryId : Text; name : Text; pointsPossible : Nat; dueDate : ?Text };
  ...
  type OldActor = {
    ...
    gradeCategories : Map.Map<Text, GradeCategory>;
    scores          : Map.Map<Text, Score>;
    assignmentsV2   : Map.Map<Nat, AssignmentV2>;
    state : {
  ...
  // ── NewActor — adds enrollments, classMeetings, nextMeetingId ─
  type NewActor = {
    students        : Map.Map<StudentId,      Student>;
    staff           : Map.Map<StaffId,        Staff>;
    courses         : Map.Map<CourseId,       Course>;
    ...
    scores          : Map.Map<Text, Score>;
    assignmentsV2   : Map.Map<Nat, AssignmentV2>;
    // NEW scheduling storage
    enrollments   : Map.Map<Text, Enrollment>;
    classMeetings : Map.Map<MeetingId, ClassMeeting>;
    // NEW auth registry (principal -> account)
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      gradeCategories = old.gradeCategories;
      scores          = old.scores;
      assignmentsV2   = old.assignmentsV2;
      // NEW empty maps
      enrollments   = Map.empty<Text, Enrollment>();
      classMeetings = Map.empty<MeetingId, ClassMeeting>();
      // NEW empty auth registry
      ...
      state = {
        ...
        var nextAssignmentV2Id      = old.state.nextAssignmentV2Id;
        var nextCategoryId          = old.state.nextCategoryId;
        var nextScoreId             = old.state.nextScoreId;
