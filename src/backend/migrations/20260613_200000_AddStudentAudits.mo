// Migration: add studentAudits stable map to actor state.
// OldActor = NewActor of 20260611_120000_InitEdUniteOS.mo (no studentAudits).
// NewActor = adds studentAudits : Map.Map<AuditId, AuditEntry>.

import Map "mo:core/Map";

module {

  // ── Inline types (self-contained; no project imports allowed) ────────
  ...
  // Separate audit entry type used by studentAudits map (richer, Text ids)
  ...
  // ── Migration contract ────────────────────────────────────────────────
  // OldActor = NewActor of 20260611_120000_InitEdUniteOS (no studentAudits)
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      threads       = old.threads;
      signals       = old.signals;
      studentAudits = Map.empty<AuditId, StudentAuditEntry>();
      state         = old.state;
