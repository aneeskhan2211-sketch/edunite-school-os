// Migration: Add ConferenceBooking, Extracurriculars, and StaffRoom stable maps
// and counters.
// OldActor = NewActor of 20260615_200000_AddAllNewDomains.mo
...
module {
  ...
  // ── Shared types (inlined) — must exactly match AddAllNewDomains.mo NewActor ─
  type Role = {
  ...
  // StudentAuditEntry matches types/students.mo AuditEntry (field delta not details)
  ...
  // ── NewActor — adds all three new domain maps and counters ────────────
  type NewActor = {
    students        : Map.Map<StudentId,      Student>;
    staff           : Map.Map<StaffId,        Staff>;
    courses         : Map.Map<CourseId,       Course>;
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      // New maps — initialised empty
      conferenceSlots    = Map.empty<ConferenceSlotId,    ConferenceSlot>();
      conferenceBookings = Map.empty<ConferenceBookingId, ConferenceBooking>();
      activities         = Map.empty<ActivityId,          Activity>();
      activityRosters    = Map.empty<Text,                ActivityRoster>();
      serviceHours       = Map.empty<ServiceEntryId,      ServiceHoursEntry>();
      fieldTrips         = Map.empty<FieldTripId,         FieldTrip>();
      permissionSlips    = Map.empty<PermissionSlipId,    PermissionSlip>();
      channels           = Map.empty<ChannelId,           Channel>();
      channelMsgs        = Map.empty<ChannelMsgId,        ChannelMessage>();
      boardItems         = Map.empty<BoardItemId,         BoardItem>();
      state = {
        ...
        var nextConferenceSlotId    = 1;
        var nextConferenceBookingId = 1;
        var nextActivityId          = 1;
        var nextServiceEntryId      = 1;
        var nextFieldTripId         = 1;
        var nextPermissionSlipId    = 1;
        var eligibilityGpaMin       = 2.0;
        var eligibilityAttMin       = 85.0;
        var nextChannelId           = 1;
        var nextChannelMsgId        = 1;
        var nextBoardItemId         = 1;
      };
