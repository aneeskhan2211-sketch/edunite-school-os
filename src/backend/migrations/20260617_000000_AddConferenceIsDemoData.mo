// Migration: Add isDemoData = false to all existing conferenceSlots and conferenceBookings
// OldActor = NewActor of 20260615_210000_AddConferenceExtracurricularStaffRoom.mo
...
module {
  ...
  // ── Shared types (inlined) — must exactly match 20260615_210000 NewActor ─
  type Role = {
  ...
  // ── Old domain types (without isDemoData) ─────────────────────────────
  type OldConferenceSlot = {
    id              : ConferenceSlotId;
    teacherId       : Text;
    studentId       : Text;
    parentId        : Text;
  ...
  // ── New domain types (with isDemoData) ────────────────────────────────
  type NewConferenceSlot = {
    id              : ConferenceSlotId;
    teacherId       : Text;
    studentId       : Text;
    parentId        : Text;
  ...
  type OldActor = {
    ...
    // Conference booking
    conferenceSlots    : Map.Map<ConferenceSlotId,    OldConferenceSlot>;
    conferenceBookings : Map.Map<ConferenceBookingId, OldConferenceBooking>;
    // Extracurriculars
  ...
  // ── NewActor — transforms conferenceSlots and conferenceBookings to add isDemoData ─
  type NewActor = {
    students        : Map.Map<StudentId,      Student>;
    staff           : Map.Map<StaffId,        Staff>;
    courses         : Map.Map<CourseId,       Course>;
    ...
    interventions   : Map.Map<InterventionId, Intervention>;
    appointments    : Map.Map<AppointmentId,  Appointment>;
    // Conference booking — with isDemoData
    conferenceSlots    : Map.Map<ConferenceSlotId,    NewConferenceSlot>;
    conferenceBookings : Map.Map<ConferenceBookingId, NewConferenceBooking>;
    // Extracurriculars
    activities      : Map.Map<ActivityId,       Activity>;
    activityRosters : Map.Map<Text,             ActivityRoster>;
    serviceHours    : Map.Map<ServiceEntryId,   ServiceHoursEntry>;
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      // Transform conferenceSlots to add isDemoData = false
      conferenceSlots = old.conferenceSlots.map<ConferenceSlotId, OldConferenceSlot, NewConferenceSlot>(
        func(_id, slot) { { slot with isDemoData = false } }
      );
      // Transform conferenceBookings to add isDemoData = false
      conferenceBookings = old.conferenceBookings.map<ConferenceBookingId, OldConferenceBooking, NewConferenceBooking>(
        func(_id, booking) { { booking with isDemoData = false } }
      );
