// Migration: Add gradebook V2 storage (GradeCategory, Score maps + counters)
// OldActor = NewActor of 20260617_010000_AddCommitmentNotesAndTransitionLog.mo
...
module {
  ...
  // ── New gradebook V2 types (inlined) ──────────────────────────────────
  type GradeCategory = {
    id      : Text;
    classId : CourseId;
    name    : Text;
  ...
  // ── NewActor — adds gradeCategories, scores, nextCategoryId, nextScoreId ─
  type NewActor = {
    students        : Map.Map<StudentId,      Student>;
    staff           : Map.Map<StaffId,        Staff>;
    courses         : Map.Map<CourseId,       Course>;
    ...
    channelMsgs : Map.Map<ChannelMsgId, ChannelMessage>;
    boardItems  : Map.Map<BoardItemId,  BoardItem>;
    // NEW gradebook V2 storage
    gradeCategories : Map.Map<Text, GradeCategory>;
    scores          : Map.Map<Text, Score>;
    assignmentsV2   : Map.Map<Nat, { id : Nat; classId : CourseId; categoryId : Text; name : Text; pointsPossible : Nat; dueDate : ?Text }>;
    state : {
      var nextStudentId           : Nat;
      var nextStaffId             : Nat;
      var nextCourseId            : Nat;
  ...
  public func migration(old : OldActor) : NewActor {
    {
      ...
      // NEW empty maps
      gradeCategories = Map.empty<Text, GradeCategory>();
      scores          = Map.empty<Text, Score>();
      assignmentsV2   = Map.empty<Nat, { id : Nat; classId : CourseId; categoryId : Text; name : Text; pointsPossible : Nat; dueDate : ?Text }>();
      state = {
